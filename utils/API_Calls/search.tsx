...
async function fetch_student_data() {
  //WILL throw errors when something goes wrong - this is intentional
...
async function start_IDB() {
  //if this resolves, the global variable 'db' should contain a reference to the database - otherwise it should remain an empty string
  ...
  return new Promise(async (resolve, reject) => {
    ...
    trxn.objectStore("students").openCursor().onsuccess = (event: any) => {
      ...
      if (cursor) {
        ...
        if (!Array.isArray(cursor.value.students)) {
